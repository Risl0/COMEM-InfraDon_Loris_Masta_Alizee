-- Active: 1773408571646@@127.0.0.1@5432@service_technique
...
FROM (
        SELECT
            CASE
                ...
                OR LOWER(type) LIKE '%borne%' THEN 'borne recharge'
...
FROM (
        SELECT
            CASE
                ...
                WHEN LOWER(etat) LIKE '%usé%'
                OR LOWER(etat) LIKE '%use%' THEN 'usé'
                ELSE TRIM(etat)
...
FROM (
        SELECT
            CASE
                ...
                WHEN LOWER(statut) LIKE '%en cours%'
                OR LOWER(statut) LIKE '%encours%' THEN 'en cours'
                ELSE TRIM(statut)
...
FROM
    ...
    LEFT JOIN type_mobilier tm ON (
        CASE
            ...
            WHEN LOWER(i.type) LIKE '%borne recharge%'
            OR LOWER(i.type) LIKE '%borne ev%'
            OR LOWER(i.type) LIKE '%borne%' THEN 'borne recharge'
            ...
            WHEN LOWER(i.type) LIKE '%poubelle%'
            OR LOWER(i.type) LIKE '%corbeille%' THEN 'poubelle'
            ELSE TRIM(i.type)
        END
    ) = tm.libelle;
...
SELECT
    ...
    CASE
        ...
        WHEN TRIM(duree) ~ '^\d+h\d{2}$' THEN (
            SPLIT_PART(TRIM(duree), 'h', 1)::INT * 60 + SPLIT_PART(TRIM(duree), 'h', 2)::INT
        ) / 60
...
FROM staging.intervention s
    LEFT JOIN type_intervention ti ON (
        CASE
            WHEN LOWER(s.objet) LIKE '%réparation%'
            OR LOWER(s.objet) LIKE '%reparation%' THEN 'réparation'
            ...
            WHEN LOWER(s.objet) LIKE '%détartrage%'
            OR LOWER(s.objet) LIKE '%detartrage%' THEN 'détartrage'
            WHEN LOWER(s.objet) LIKE '%mise à jour%'
            OR LOWER(s.objet) LIKE '%mise a jour%' THEN 'mise à jour logiciel'
            ELSE 'autre'
        END
    ) = ti.libelle;
...
INSERT INTO
    intervention_inventaire_mobilier (
        id_intervention,
...
FROM
    ...
    JOIN inventaire_mobilier im ON (
        ...
        OR i.objet ILIKE '%' || COALESCE(
            (
                SELECT libelle
                FROM type_mobilier
                WHERE
                    id = im.id_type_mobilier
            ),
            ''
        ) || '%'
    );
...
INSERT INTO
    signalement_inventaire_mobilier (
        id_signalement,
        id_inventaire_mobilier
    )
SELECT DISTINCT
...
SELECT
    ...
    SUM(
        CASE
            WHEN LOWER(s.urgence) = 'urgent' THEN 1
            ELSE 0
        END
    ) AS nb_signalements_urgents,
    SUM(
        CASE
            WHEN LOWER(s.urgence) = 'normal' THEN 1
            ELSE 0
        END
    ) AS nb_signalements_normaux
FROM
    signalement s
...
ALTER TABLE staging.fournisseur_contact
ADD COLUMN IF NOT EXISTS type_materiel TEXT;

UPDATE fournisseur_contact
SET
    id_type_materiel = tm.id
FROM type_materiel tm
WHERE (
        fournisseur_contact.entreprise = 'Mobilier Urbain Suisse SA'
        AND tm.libelle = 'bois'
    )
    OR (
        fournisseur_contact.entreprise = 'Eclairage Public Romandie'
        AND tm.libelle = 'métal'
    )
    OR (
        fournisseur_contact.entreprise = 'GreenCharge SA'
        AND tm.libelle = 'métal'
    )
    OR (
        fournisseur_contact.entreprise = 'Bächler Metallbau AG'
        AND tm.libelle = 'métal'
    )
    OR (
        fournisseur_contact.entreprise = 'Holz Design GmbH'
        AND tm.libelle = 'bois'
    );

UPDATE public.inventaire_mobilier
SET
    id_fournisseur_contact = fc.id
FROM fournisseur_contact fc
WHERE (
        inventaire_mobilier.id_type_materiel = (
            SELECT id
            FROM type_materiel
            WHERE
                libelle = 'bois'
        )
        AND fc.entreprise = 'Holz Design GmbH'
    )
    OR (
        inventaire_mobilier.id_type_materiel = (
            SELECT id
        ...
        AND fc.entreprise = 'Bächler Metallbau AG'
    )
    OR (
        inventaire_mobilier.id_type_mobilier = (
            SELECT id
            FROM type_mobilier
            WHERE
                libelle = 'lampadaire'
        )
        AND fc.entreprise = 'Eclairage Public Romandie'
    )
    OR (
        inventaire_mobilier.id_type_mobilier = (
            SELECT id
            FROM type_mobilier
            WHERE
                libelle = 'borne recharge'
        )
        AND fc.entreprise = 'GreenCharge SA'
    )
    OR (
        inventaire_mobilier.id_type_mobilier = (
            SELECT id
        ...
        AND fc.entreprise = 'Fontaines & Patrimoine Sàrl'
    );

UPDATE public.inventaire_mobilier
SET
    id_type_materiel = tm.id
FROM staging.inventaire si
    JOIN type_materiel tm ON (
        CASE
            WHEN LOWER(si.materiau) LIKE '%métal%'
            OR LOWER(si.materiau) LIKE '%metal%' THEN 'métal'
            WHEN LOWER(si.materiau) LIKE '%bois%' THEN 'bois'
            WHEN LOWER(si.materiau) LIKE '%pierre%' THEN 'pierre'
            WHEN LOWER(si.materiau) LIKE '%béton%' THEN 'béton'
            ELSE TRIM(si.materiau)
        END
    ) = tm.libelle
WHERE
    inventaire_mobilier.lieu = NULLIF(TRIM(si.lieu), '')
    AND inventaire_mobilier.id_type_materiel IS NULL;

DELETE FROM public.inventaire_mobilier
WHERE
    id IN (
        SELECT id
        FROM (
                SELECT id, ROW_NUMBER() OVER (
                        PARTITION BY
                            LOWER(TRIM(lieu)), id_type_mobilier, date_installation
                        ORDER BY latitude NULLS LAST
                    ) AS rn
...
SET
    cout_materiel = 0.00
WHERE
    remarque ILIKE '%garantie%'
    AND cout_materiel IS NULL;

ALTER TABLE staging.intervention
ADD COLUMN IF NOT EXISTS type_intervention_csv TEXT;